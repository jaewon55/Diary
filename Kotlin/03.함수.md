...
```
이름이 없는 함수, 일반 익명 함수에서는 return, break, continue가 사용 가능하지만 람다식에서는 사용하기 어렵다.(라벨 표기법과 같이 사용해야 한다.)

+ 인라인 함수
```kotlin
inline fun sumLambda(sum: (Int, Int) -> Int, a: Int, b: Int) = sum(a, b) // 인라인 함수 선언
```
함수는 호출될 때 마다 분기가 발생한다. 분기가 발생하면 새로운 스택프레임이 생기면서 메모리상 비용이든다.(성능저하)   
